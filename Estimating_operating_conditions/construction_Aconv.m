function [A_conv] = construction_Aconv_v1(G)
...
k_total_P = k_turb_P; % constant k_cond removed
k_total_e = k_turb_e; 
k_total_w = k_turb_w; % symmetry
...
k_turb_e = 2*k_turb_P*k_turb_E/(k_turb_P+k_turb_E);
k_turb_w = 2*k_turb_P*k_turb_W/(k_turb_P+k_turb_W);
k_total_P = k_turb_P; % constant k_cond
k_total_e = k_turb_e; 
k_total_w = k_turb_w; % symmetry
v_P = vz_profile(index_x);
...
column_k_total_P = column_k_turb_P; % constant k_cond
column_k_total_e = column_k_turb_e; 
column_k_total_w = column_k_turb_w; 
...
%% resulting matrix is advection (i.e., all conduction term removed)
A_conv = A;
